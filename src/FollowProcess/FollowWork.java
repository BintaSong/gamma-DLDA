...
public class FollowWork{
		...
		try{
			...
			for(m = 0;m < corpus.M;m++){
				...
				for(k = 0;k < corpus.K;k++){
					...
					//加入伪参数alpha更好
					corpus.phi[m][k] = (double)(corpus.nmk[m][k]+corpus.alpha)/(corpus.nmkSum[m]+corpus.K * corpus.alpha); 
					
					//System.out.println("nmk[m][k]" + (double)corpus.nmk[m][k]);
				}
	...
	public void writeTheta(){
		...
		try{
			...
			System.out.println("写入 theta 文档错误");
			e.printStackTrace();				
		}finally{
			if (writer != null) {
			try {
					...
					System.out.println("关闭 theta文档错误!");
					e.printStackTrace();
				}
			}
		}
